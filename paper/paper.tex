...
\begin{itemize}
  ...
  \item Hidden layer with tanh activation, of dimension $\left\lfloor \sqrt{s_s \, s_a} \right\rfloor$
  \item Output layer with tanh activation, followed by log softmax, of dimension $s_a$
\end{itemize}
During training, we generally use a learning rate of $\alpha = 0.01$. 


\subsubsection{REINFORCE Policy Gradient******}
Perhaps the most basic policy gradient method is the REINFORCE algorithm. This algorithm uses a Monte Carlo-type approach to gathering training data. During each episode, the agent follows 
